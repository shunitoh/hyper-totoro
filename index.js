...
const colorSchemes = {
  totoro: path.resolve(__dirname, 'themes', 'totoro.yml')
};

function getUserOptions(configObj) {
  return Object.assign({}, {
    get totoro() {
      if (Array.isArray(configObj.totoro)) {
        return configObj.totoro[Math.floor(Math.random() * configObj.totoro.length)];
      }
      return configObj.totoro || 'dark';
    },
    get totorotab() {
      return (configObj.totorotab || 'false') === 'true';
    },
    get unibody() {
...
function getThemeColors(theme) {
  ...
  if (name === 'random') {
    return getRandomTheme(themes.totoro);
  }
  ...
  if (Object.prototype.hasOwnProperty.call(themes.totoro, name)) {
    // Return the requested totoro theme -- i.e. `lapras`
    return [name, themes.totoro[name]];
  }
  // Got non-existent theme name thus resolve to default
  return ['dark', themes.totoro.dark];
}
...
exports.decorateConfig = config => {
  ...
  const [themeName, colors] = getThemeColors(options.totoro);
  ...
  // Set totorotab
  const tabContent = options.totorotab ? gifPath : '';
